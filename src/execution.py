...
def budgeted_size(
    beta, spots, deltas, prices, budget, max_error, slippage_bps, commission
):
    ...
    """
    costs = (
        CONTRACT_MULTIPLIER * np.asarray(prices) * (1 + slippage_bps / 10000)
        + commission
    )
    ...
    for nd in range(1, max(0, int(np.floor((budget - costs[1]) / costs[0]))) + 1):
        ...
        high = min(
            int(np.floor((1 + max_error) * ratio * nd + 1e-12)),
...
def entry_option_terms(instruction, date, prices, volatility, risk_free_rates):
    ...
    spots = [
        float(prices.at[date, instruction[l]]) for l in ("dependent", "independent")
    ]
    vols = [
        float(volatility.at[date, instruction[l]]) for l in ("dependent", "independent")
    ]
    ...
    deltas = [
        black_scholes_delta(s, s, T, rf, v, k) for s, v, k in zip(spots, vols, types)
    ]
...
def run_backtest(
    ...
    ResearchConfig(
        ...
        premium_budget_fraction=premium_budget_fraction,
    ...
    if (
        not np.isfinite(
            params[["alpha", "beta", "mu", "kappa", "sigma", "hurst", "variance"]]
        )
        .all()
        .all()
    ):
    ...
    equity = (
        []
    )  # First OOS close is the all-cash initial valuation; first fill is the next close.
    ...
    def quote(pos, date):
        ...
        gross = CONTRACT_MULTIPLIER * sum(
            pos[leg + "_contracts"] * p
            for leg, p in zip(("dependent", "independent"), px)
        )
    ...
    for date in test.index:
        ...
        for pair in list(opened):
            ...
            lagged_cross = _has_converged(
                spreads[pair].iloc[i - 1], pos["mu"], pos["direction"]
            )
                ...
                trades.append(
                    {
                        **pos,
                        ...
                        "exit_reason": (
                            "expiry"
                            if expired
                            else ("end_of_test" if last else "convergence")
                        ),
                        ...
                        "holding_trading_days": int(
                            i - dates.get_loc(pos["entry_date"])
                        ),
                        "holding_calendar_days": int((date - pos["entry_date"]).days),
                    }
                )
                del opened[pair]
        # With the default fraction of one, each entry may use all remaining cash.
        # Optional smaller fractions use equity marked before today's entries.
        ...
        for pair in sorted(pending):
            ...
            if pair in opened:
                reason = "already_open"
            elif max_open_pairs is not None and len(opened) >= max_open_pairs:
                reason = "max_open_pairs"
            elif date >= instruction["expiry_date"]:
            ...
            budget = max(0.0, min(cash, nav * premium_budget_fraction))
            ...
            gross = CONTRACT_MULTIPLIER * sum(
                pos[l + "_contracts"] * p
                for l, p in zip(("dependent", "independent"), px)
            )
            entry_cost = gross * slip + commission_per_contract * (
                pos["dependent_contracts"] + pos["independent_contracts"]
            )
        ...
        if date != test.index[-1]:
            for row in params.itertuples():
                ...
                if abs(z) < entry_z or len(history) < memory_window + 1:
                    continue
                ...
                rec = dict(
                    ...
                    target_probability=target_probability,
                    probability_at_selected_horizon=float(
                        sig.probability_at_selected_dte
                    ),
                ...
                if i + h >= len(dates):
                    skipped.append(
                        dict(date=date, pair=row.pair, reason="horizon_beyond_test")
                    )
                    continue
                ...
                if h <= 1:
                    skipped.append(
                        dict(date=date, pair=row.pair, reason="no_remaining_maturity")
                    )
                    continue
                pending[row.pair] = rec
        mark = sum(quote(p, date)[0] for p in opened.values())
        if cash < -1e-7 or (
            max_open_pairs is not None and len(opened) > max_open_pairs
        ):
    ...
    return dict(
        ...
        skipped_signals=pd.DataFrame(skipped),
