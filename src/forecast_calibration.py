...
def label_forecasts(forecasts, prices):
    ...
    for r in forecasts.to_dict('records'):
        ...
        values = np.log(future[r['dependent']])-r['alpha']-r['beta']*np.log(future[r['independent']])
        ...
        rows.append({**r,'event_status':status,'selected_horizon_assessable':bool(success or complete),
                     'complete_horizon_observed':complete,'observed_path_steps':prefix,
                     ...
                     'realized_within_selected_horizon':y,'brier_component':(y-p)**2,
...
def summarize_forecasts(labels):
    ...
    return dict(n_forecasts=len(labels),n_complete_horizon=len(full),
