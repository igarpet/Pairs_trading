...
def prices_fixture():
    ...
    b = 100 * np.exp(np.cumsum(rng.normal(0, 0.007, len(dates))))
    a = b * np.exp(0.10 + rng.normal(0, 0.01, len(dates)))
    ...
    c = pd.DataFrame([dict(pair="A-B", alpha=0.0, beta=1.0)])
...
def fake_signal(*args, **kw):
    return (
        SimpleNamespace(
            selected_dte_trading_days=4,
            direction=1,
            probability_at_selected_dte=0.75,
            probability_at_max_horizon=0.9,
        ),
        None,
    )
...
def test_next_close_budget_costs_and_exact_maturity(monkeypatch):
    ...
    r = run_backtest(train, test, pairs, c, 0.03)
    tr = r["trades"]
    assert len(tr) > 0
    ...
    for row in tr.itertuples():
        ...
        assert row.relative_hedge_error <= 0.1 + 1e-10
    ...
    assert eq.equity.iloc[-1] == pytest.approx(
        ResearchConfig().initial_capital + tr.pnl.sum()
    )
...
def test_decisions_do_not_use_execution_day_close(monkeypatch):
    monkeypatch.setattr("src.execution.calculate_convergence_signal", fake_signal)
    train, test, pairs, c = prices_fixture()
    one = run_backtest(train, test, pairs, c, 0.03)
    changed = test.copy()
    changed.iloc[1, 0] *= 1.04
    two = run_backtest(train, changed, pairs, c, 0.03)
...
def test_default_allocation_uses_five_percent_budget(monkeypatch):
    monkeypatch.setattr("src.execution.calculate_convergence_signal", fake_signal)
    train, test, pairs, c = prices_fixture()
    cfg = ResearchConfig().validate()
    assert cfg.premium_budget_fraction == 0.05
    assert cfg.max_open_pairs is None
    result = run_backtest(train, test, pairs, c, 0.03)
    first = result["trades"].iloc[0]
    assert first.entry_budget == pytest.approx(
        cfg.initial_capital * cfg.premium_budget_fraction
    )
    assert first.entry_premium > 0.04 * cfg.initial_capital
...
def test_convergence_exit_is_delayed(monkeypatch):
    ...
    train, test, pairs, c = prices_fixture()
    test = test.copy()
    test.iloc[2, 0] = test.iloc[2, 1] * 0.9
    r = run_backtest(train, test, pairs, c, 0.03)
...
def test_limit_and_order_independent_of_dataframe(monkeypatch):
    ...
    c = pd.concat(
        [c, pd.DataFrame([dict(pair="C-D", alpha=0.0, beta=1.0)])], ignore_index=True
    )
    a = run_backtest(train, test, allpairs, c, 0.03, max_open_pairs=1)
    b = run_backtest(train, test, allpairs.iloc[::-1], c, 0.03, max_open_pairs=1)
...
def test_sizing_matches_exhaustive_integer_reference():
    spots = np.array([120.0, 90.0])
    deltas = np.array([-0.4, 0.6])
    prices = np.array([3.0, 2.0])
    s = budgeted_size(1.2, spots, deltas, prices, 5000, 0.1, 10, 0.65)
    ratio = 1.2 * 48 / 54
    costs = 100 * prices * 1.001 + 0.65
    feasible = []
    for a in range(1, 25):
        for b in range(1, 25):
            err = abs(b / (a * ratio) - 1)
            debit = a * costs[0] + b * costs[1]
            if err <= 0.1 and debit <= 5000:
                feasible.append((debit, -err, -(a + b), a, b))
    best = max(feasible)
    assert (s["dependent_contracts"], s["independent_contracts"]) == best[-2:]
    assert budgeted_size(1, spots, deltas, prices, 1, 0.1, 10, 0.65) is None


def test_no_trades_and_immediate_horizon(monkeypatch):
    def signal(*a, **k):
        x, _ = fake_signal()
        x.selected_dte_trading_days = 1
        return x, None
    ...
    assert (
        backtest_summary(
            r["trades"], r["equity_curve"], ResearchConfig().initial_capital
        )["total_return"]
        == 0
    )
...
def event_fixture():
    ...
    prices = pd.DataFrame(
        {"A": np.exp([0.3, 0.2, 0.1, -0.1, 0.1, 0.1]) * 100, "B": 100}, index=idx
    )
    forecast = dict(
        pair="A-B",
        dependent="A",
        ...
        direction=1,
        signal_date=idx[0],
        convergence_horizon_trading_days=4,
        probability_at_selected_horizon=0.7,
        exit_date=idx[1],
        exit_reason="expiry",
    )
...
def test_incomplete_success_and_failure_have_distinct_status():
    prices, f = event_fixture()
    f["convergence_horizon_trading_days"] = 10
...
def test_horizon_inclusive_boundary():
    prices, f = event_fixture()
    f["convergence_horizon_trading_days"] = 2
...
def test_membership_known_at_cutoff():
    ...
    m = pd.DataFrame(
        dict(
            ticker=["A", "B", "C"],
            member_from=["2010-01-01"] * 3,
            member_to=[None] * 3,
            known_at=["2010-01-01", "2010-01-01", "2030-01-01"],
        )
    )
...
def test_cointegration_fixed_orientation_and_unadjusted_metadata():
    ...
    x = 4 + np.cumsum(rng.normal(0, 0.01, 500))
    y = x + rng.normal(0, 0.004, 500)
    p = pd.DataFrame(
        {"A": np.exp(y), "B": np.exp(x)},
...
def test_option_expiry_and_parity():
    for kind in ["call", "put"]:
        p = black_scholes_price(110, 100, 0, 0.03, 0.2, kind)
        assert p == (10 if kind == "call" else 0)
    c = black_scholes_price(110, 100, 1, 0.03, 0.2, "call")
    p = black_scholes_price(110, 100, 1, 0.03, 0.2, "put")
    assert c - p == pytest.approx(110 - 100 * np.exp(-0.03))


def test_alignment_uses_previous_rate_and_initial_equity():
    idx = pd.bdate_range("2024-01-01", periods=4)
    e = pd.DataFrame({"equity": [100, 101, 102, 103]}, index=idx)
    b = pd.Series([100, 102, 99, 103], index=idx)
    rates = pd.Series([0.01, 0.02, 0.03, 0.04], index=idx)
    a = aligned_returns(e, b, rates)
    assert len(a) == 3
    assert a.rf_daily.iloc[0] == pytest.approx(1.01 ** (1 / 252) - 1)
    with pytest.raises(ValueError, match="cover"):
        aligned_returns(e, b.iloc[1:], rates)
...
def test_placebo_small_pool_terminates_and_repeatability():
    ...
    out = compare_placebos(
        {"total_return": 0.1}, pd.DataFrame({"total_return": [0.1, 0.2, 0]})
    )
    assert out.iloc[0].p_value == 0.75


def test_bootstrap_reproducible_and_constant_return():
    a = bootstrap_mean(
        np.ones(60) * 0.01, blocks=(10, 20, 100), replications=100, seed=2
    )
    b = bootstrap_mean(
        np.ones(60) * 0.01, blocks=(10, 20, 100), replications=100, seed=2
    )
    pd.testing.assert_frame_equal(a, b)
    assert a.iloc[0].ci_low == pytest.approx(0.01)
...
def test_fixed_settings_and_inputs_can_be_overwritten(tmp_path, monkeypatch):
    ...
    pd.DataFrame({"rate": [0.03]}).to_parquet(rates)
    member = tmp_path / "membership.csv"
    member.write_text("ticker,member_from,member_to,known_at\n")
    io.initialize(ResearchConfig(), prices, rates, rates, member)
    pd.DataFrame({"A": [123.0]}).to_parquet(prices)
    io.initialize(ResearchConfig(entry_z=2.0), prices, rates, rates)
    assert io.load_config().entry_z == 2.0
    assert pd.read_parquet(io.OUTPUT_DIR / "inputs/prices.parquet").A.iloc[0] == 123.0
...
def test_raw_cointegration_selection_is_not_multiplied_by_family(monkeypatch):
    rng = np.random.default_rng(132)
    x = 4 + np.cumsum(rng.normal(0, 0.01, 300))
    y = x + rng.normal(0, 0.02, 300)
    p = pd.DataFrame(
        {"A": np.exp(y), "B": np.exp(x), "C": np.exp(x + 0.1)},
        index=pd.bdate_range("2020-01-01", periods=300),
    )
    # A raw p=.005 passes at 1%, whereas Holm over three pairs would fail.
    monkeypatch.setattr(
        "src.cointegration.coint", lambda *a, **k: (-4.0, 0.005, [-4.0, -3.0, -2.0])
    )
    monkeypatch.setattr(
        "src.cointegration.adfuller",
        lambda v, **k: (-3.0, 0.5 if len(v) == 300 else 0.001, 0, 0, {}, 0),
    )
    selected, _, a = screen_cointegration(p, [("A", "B")])
    assert a.n_family_tests.iloc[0] == 3
    assert a.adjusted_pvalue.iloc[0] == pytest.approx(0.005)
    assert a.multiplicity_method.iloc[0] == "none"
    assert selected.pair.tolist() == ["A-B"]
    rejected, _, _ = screen_cointegration(p, [("A", "B")], significance=0.001)
...
def test_cached_forecasts_are_identical_to_uncached(monkeypatch):
    ...
    train, test, pairs, c = prices_fixture()
    cache = {}
    a = run_backtest(train, test, pairs, c, 0.03, signal_cache=cache)
    assert cache

    def fail(*a, **k):
        raise AssertionError("Identical forecast must be cached")

    monkeypatch.setattr("src.execution.calculate_convergence_signal", fail)
    b = run_backtest(train, test, pairs, c, 0.03, signal_cache=cache)
    pd.testing.assert_frame_equal(a["trades"], b["trades"])
