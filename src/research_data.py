...
def prepare_prices(prices, config, membership=None, allow_legacy=False):
    ...
    if membership is not None:
        ...
        if not required.issubset(membership):
            raise ValueError(f"Membership requires {sorted(required)}")
        m = membership.copy()
        for c in ["member_from", "member_to", "known_at"]:
            m[c] = pd.to_datetime(m[c])
        if m[["member_from", "known_at"]].isna().any().any():
            raise ValueError("Membership dates cannot be missing.")
        keep = m.loc[
