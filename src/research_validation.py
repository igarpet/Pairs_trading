...
def aligned_returns(equity, benchmark, rates):
    ...
    if b.isna().any() or (b <= 0).any() or not np.isfinite(b).all():
        raise ValueError(
            "Frozen benchmark must cover every equity date including the initial date."
        )
...
def market_regression(aligned, lags=5):
    ...
    out = dict(
        ...
        alpha_t_stat=t,
        alpha_p_value_two_sided=p,
        alpha_p_value_one_sided_positive=p / 2 if t > 0 else 1 - p / 2,
...
def bootstrap_mean(returns, blocks=(10, 20, 40), replications=10000, seed=42):
    ...
    for b in blocks:
        if b > len(x):
            rows.append(dict(block_length=b, status="insufficient_observations"))
            continue
...
def compare_placebos(actual, placebos):
    ...
    for key in ["total_return", "daily_sharpe", "average_trade_return"]:
        a = actual.get(key)
        values = (
            pd.to_numeric(placebos[key], errors="coerce")
            if key in placebos
