...
ORDER=['01_Stock_Data.ipynb','01_RF_Data.ipynb','02_Pair_Selection.ipynb',
       '03_fractional_OU.ipynb','03_pair_eligibility.ipynb','04_convergence_signal.ipynb',
       '05_volatility_model.ipynb','06_option_pricing.ipynb','07_backtest.ipynb',
       '08_drawdown_diagnostics.ipynb','09_systematic_risk_diagnostics.ipynb',
       '10_equilibrium_shift_diagnostics.ipynb','11_static_convergence_calibration.ipynb',
...
"""
    ...
    repo=tmp_path/'notebook_repo';repo.mkdir()
    for name in ['src','scripts']:
        shutil.copytree(root/name,repo/name,ignore=shutil.ignore_patterns('__pycache__'))
    for name in ORDER+['requirements.txt','requirements-notebooks.txt','METHODOLOGY.md']:
        shutil.copyfile(root/name,repo/name)
    n=400;idx=pd.bdate_range('2020-01-01',periods=n)
    x=4.5+np.cumsum(np.random.default_rng(51).normal(0,.015,n))
    noise=simulate_unconditional_fou_paths(0,0,.2,.015,.3,n-1,n_paths=1,seed=32)[0]
    prices=pd.DataFrame({'A':np.exp(x+noise),'B':np.exp(x)},index=idx)
    data=repo/'data/processed';(data/'systematic_risk').mkdir(parents=True)
    prices.to_parquet(data/'prices.parquet')
    pd.DataFrame({'risk_free_rate':.03},index=idx).to_parquet(data/'risk_free_rates.parquet')
    prices[['B']].to_parquet(data/'systematic_risk/sp500_prices.parquet')
    first=repo/ORDER[0];nb=nbformat.read(first,as_version=4)
    for c in nb.cells:
        if c.cell_type=='code':
            c.source=c.source.replace('CONFIG = ResearchConfig()',
